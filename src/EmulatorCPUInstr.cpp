...
WORD Emulator::StackPop() {
	WORD val = memory.ReadWord(cpu.SP);
	cpu.SP += 2;
	return val;
}
...
void Emulator::RLC_n() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in RLC()");
            std::cin.get();
        }
	}
}
...
void Emulator::RRC_n() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in RRC()");
            std::cin.get();
        }
	}
}
...
void Emulator::SLA_n() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in SLA()");
            std::cin.get();
        }
	}
}
void Emulator::SRA_n() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in SRA()");
            std::cin.get();
        }
	}
}
void Emulator::SRL_n() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in SRL()");
            std::cin.get();
        }
	}
}
...
void Emulator::BIT_b_r() {
	...
	switch (reg) {
		case 0x0:
		{
			cpu.SetFlags(TestBit(cpu.B, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x1:
		{
			cpu.SetFlags(TestBit(cpu.C, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x2:
		{
			cpu.SetFlags(TestBit(cpu.D, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x3:
		{
			cpu.SetFlags(TestBit(cpu.E, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x4:
		{
			cpu.SetFlags(TestBit(cpu.H, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x5:
		{
			cpu.SetFlags(TestBit(cpu.L, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x6:
		{
			cpu.SetFlags(TestBit(memory.mem[cpu.HL], bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
		case 0x7:
		{
			cpu.SetFlags(TestBit(cpu.A, bit) == 0, 0, 1, cpu.GetFlagC());
			break;
		}
        default:
        {
            printf("Error: invalid op in BIT()");
            std::cin.get();
        }
	}
}
void Emulator::SET_b_r() {
	...
	switch (reg) {
        ...
        default:
        {
            printf("Error: invalid op in SET()");
            std::cin.get();
        }
	}
}
void Emulator::RES_b_r() {
	...
	switch (reg) {
        ...
        default:
        {
            printf("Error: invalid op in RES");
            std::cin.get();
        }
	}
}
...
void Emulator::JP_cc_nn() { // return cycles
	...
	switch (op) {
		case 0xC2:
		{
			if (!cpu.GetFlagZ()) { cpu.PC = addr; return; }
            break;
		}
		case 0xCA:
		{
		...
		case 0xD2:
		{
			if (!cpu.GetFlagC()) { cpu.PC = addr; return; }
		    break;
		}
		case 0xDA:
		{
        ...
        default:
        {
            printf("Error: invalid op in JP_cc_nn()");
            std::cin.get();
        }
	}
    cpu.PC += 2;
...
void Emulator::JR_n() { // SIGNED VALUE
	cpu.PC += (SIGNED_BYTE)FetchOpcode();
}
void Emulator::JR_cc_n() {
    ...
    switch (op) {
		...
		{
			if (!cpu.GetFlagZ()) {
                cpu.PC += (SIGNED_BYTE)memory.mem[cpu.PC-1];
		        return;
            }
            break;
        }
        ...
        default:
        {
            printf("Error: invalid op in JR_cc_nn()");
            std::cin.get();
        }
	}
}
...
void Emulator::CALL_nn() {
	WORD addr = memory.ReadWord(cpu.PC);
	StackPush(cpu.PC+2); // push addr of next instruction onto stack
    cpu.PC = addr; // jump to the address read
}
...
void Emulator::CALL_cc_nn() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in CALL_cc_nn()");
            std::cin.get();
        }
	}
}
...
void Emulator::RET_cc() {
	...
	switch (op) {
        ...
        default:
        {
            printf("Error: invalid op in RET_cc()");
            std::cin.get();
        }
	}
}
